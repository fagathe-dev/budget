...
const initForm = () => {
  ...
  expenseForm
    .querySelector('[name="category"] option[selected]')
...
const displayBudget = (budget = {}, expenses = [], container) => {
  ...
  clone
    .querySelector("[data-budget-category-icon]")
    .classList.add(budget?.category?.icon);
  clone.querySelector("[data-budget-category-name]").innerText =
    budget?.category?.name;
  clone.querySelector("[data-budget-expenses-sum]").innerText =
    sum(expenses).toFixed(2);
  clone.querySelector("[data-budget-amount]").innerText = budget?.amount;
  clone
    .querySelector("[data-budget-edit]")
    .setAttribute("href", APP_BUDGET_EDIT + budget?.id);
...
const displayExpense = (expense = {}, container) => {
  ...
  clone
    .querySelector("[data-expense-category-icon]")
    .classList.add(expense?.category?.icon);
  clone.querySelector("[data-expense-category-name]").innerText =
    expense?.category?.name;
  clone.querySelector("[data-expense-amount]").innerText =
    expense?.amount.toFixed(2);
  clone.querySelector("[data-expense-label]").innerText = expense?.label;
  clone
    .querySelector("[data-expense-edit]")
    .setAttribute("onclick", `fillExpenseForm(${expense.id})`);
  clone.querySelector("[data-expense-delete]").href =
    API_DELETE_EXPENSE + expense.id;
...
const fillExpenseForm = (id) => {
  ...
  if (expense.category) {
    form
      ...
      .querySelector(`option[value="${expense.category?.id}"]`)
...
expenseForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  return (await e.target.dataset?.action) === "create"
